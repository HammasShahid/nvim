...
```sh
git clone https://github.com/HammasShahid/nvim.git ~/.config/nvim
```
...
**NOTE** If Mason is not installing LSPs then make sure that npm is installed.

**NOTE** Checkout this file for some predefined keymaps (Some of them have been modified by me // Hammas): [keymaps](https://github.com/LunarVim/nvim-basic-ide/blob/master/lua/user/keymaps.lua)
...
Next we should should install `prettier`, `flake8`, `python-black` and other code formatters listed in null-ls

- Code formatters
```sh
sudo apt install prettier flake8 python-black
```
...
We are gonna need `fd` for the find functionality to work properly.
